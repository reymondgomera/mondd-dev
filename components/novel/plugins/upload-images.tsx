...
function isPositionValid(doc: Node, pos: number) {
  try {
    // if no error means position is valid
    doc.resolve(pos)
    return true
  } catch (err) {
    return false
...
export const createImageUpload =
  ...
  (file, view, pos) => {
    ...
    const isPosValid = isPositionValid(doc, pos)
    if (!isPosValid) return
...
export const handleImageDrop = (view: EditorView, event: DragEvent, moved: boolean, uploadFn: UploadFn) => {
  if (!moved && event.dataTransfer?.files.length) {
    ...
    const pos = coordinates?.pos ?? 0 - 1 // here we deduct 1 from the pos or else the image will create an extra node
