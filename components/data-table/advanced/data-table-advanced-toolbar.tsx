...
import DataTableCommonAction from '../data-table-common-actions'
      ...
      <div className='ml-auto flex items-center gap-2'>
        ...
        {(options.length > 0 && selectedOptions.length > 0) || openFilterBuilder ? (
          <Button variant='outline' size='sm' onClick={() => setOpenFilterBuilder(!openFilterBuilder)}>
            <Icons.chevsUpDown className='mr-2 size-4 shrink-0' aria-hidden='true' />
            Filter
          </Button>
        ) : (
          <DataTableFilterCombobox
            options={options.filter((option) => !selectedOptions.some((selectedOption) => selectedOption.value === option.value))}
            selectedOptions={selectedOptions}
            setSelectedOptions={setSelectedOptions}
            onSelect={onFilterComboboxItemSelect}
          />
        )}
        <DataTableCommonAction table={table} />
      </div>
      <div className={cn('flex items-center gap-2', !openFilterBuilder && 'hidden')}>
        ...
        {options.length > 0 && options.length > selectedOptions.length ? (
          <DataTableFilterCombobox
            ...
            onSelect={onFilterComboboxItemSelect}
          >
            <Button variant='outline' size='sm' role='combobox' className='h-7 rounded-full' onClick={() => setOpenCombobox(true)}>
              <Icons.add className='mr-2 size-4 opacity-50' aria-hidden='true' />
              Add filter
            </Button>
          </DataTableFilterCombobox>
        ) : null}
