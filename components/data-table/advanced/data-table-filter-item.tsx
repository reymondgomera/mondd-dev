...
export function DataTableFilterItem<TData>({
  ...
  const operators = selectedOption.options.length > 0 ? dataTableConfig.selectableOperators : dataTableConfig.comparisonOperators

  const [value, setValue] = React.useState(filterValues[0] ?? '')
  const debounceValue = useDebounce(value, 500)
  const [open, setOpen] = React.useState(defaultOpen)
  const [selectedOperator, setSelectedOperator] = React.useState(operators.find((c) => c.value === filterOperator) ?? operators[0])
  ...
  React.useEffect(() => {
    if (selectedOption.options.length > 0) {
      ...
      const newSearchParams = createQueryString({
        [String(selectedOption.value)]: filterValues.length > 0 ? `${filterValues.join('.')}~${selectedOperator?.value}` : null
      })
    ...
    <Popover open={open} onOpenChange={setOpen}>
      <PopoverTrigger asChild>
        <Button
          variant='outline'
          size='sm'
          className={cn('h-7 gap-0 truncate rounded-full', (selectedValues.size > 0 || value.length > 0) && 'bg-muted/50')}
        >
          <span className='font-medium capitalize'>
            {selectedOption.label} • {selectedOperator.label}
          </span>
      ...
      <PopoverContent className='w-60 space-y-1.5 p-2' align='start'>
        <div className='flex items-center space-x-1 pl-1 pr-0.5'>
          <div className='flex flex-1 items-center space-x-1'>
            ...
            <Select
              ...
              onValueChange={(value) => setSelectedOperator(operators.find((c) => c.value === value) as (typeof operators)[0])}
            >
        ...
        {selectedOption.options.length > 0 ? (
          ...
          <Input placeholder='Type here...' className='h-8' value={value} onChange={(event) => setValue(event.target.value)} autoFocus />
        )}
      </PopoverContent>
    </Popover>
  )
